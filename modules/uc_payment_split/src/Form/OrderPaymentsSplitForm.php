...
class OrderPaymentsForm extends FormBase {
  ...
  public function getFormId() {
    return 'uc_payment_split_by_order_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state, OrderInterface $uc_order = NULL) {
    ...
    $form['#attached']['library'][] = 'uc_payment_split/uc_payment_split.styles';

    $total = $this->order->getTotal();
    $payments = uc_payment_split_load_payments($this->order->id());
    ...
    foreach ($payments as $id => $payment) {
      ...
      $form['payments'][$id]['action'] = [
        ...
        '#links' => [
          'delete' => [
            'title' => $this->t('Delete'),
            'url' => Url::fromRoute('uc_payment_splits.delete', ['uc_order' => $this->order->id(), 'uc_payment_split_receipt' => $id]),
          ],
        ],
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    uc_payment_split_enter($this->order->id(), $payment['method'], $payment['amount'], $this->currentUser()->id(), NULL, $payment['comment'], $payment['received']->getTimestamp());
